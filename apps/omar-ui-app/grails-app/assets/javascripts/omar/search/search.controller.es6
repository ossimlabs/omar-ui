(function() {
    //'use strict';
    angular.module('omarApp').controller('SearchController', [
    ...
    function SearchController(coordinateConversionService, $scope, $state, wfsService, $http, stateService, toastr) {
        ...
        searchInput.autocomplete({
            ...
            serviceUrl: AppO2.APP_CONFIG.serverURL + AppO2.APP_CONFIG.params.twofishes.proxy + "/?responseIncludes=WKT_GEOMETRY_SIMPLIFIED" + "&autocomplete=true&maxInterpretations=10&autocompleteBias=BALANCED",
        ...
        $scope.$on('coordService: updated', function(event, response) {
            ...
            if (response) {
                if (response.bounds) {
                    stateService.updateMapState({bounds: response.bounds, lat: response.coordinate[1], lng: response.coordinate[0]});
                } else {
                    stateService.updateMapState({lat: response.coordinate[1], lng: response.coordinate[0]});
                }
            } else {
