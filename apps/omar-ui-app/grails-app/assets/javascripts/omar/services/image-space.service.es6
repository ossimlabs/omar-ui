(function() {
    'use strict';
    angular.module('omarApp').service('imageSpaceService', ['$rootScope', '$http', 'stateService', 'wfsService', '$timeout', '$q', '$stateParams', imageSpaceService]);

    function imageSpaceService($rootScope, $http, stateService, wfsService, $timeout, $q, $stateParams) {
        ...
        this.initImageSpaceMap = function(params) {
            ...
            map = new ol.Map({
                controls: ol.control.defaults().extend([new RotateNorthControl(), new RotateUpControl(), fullScreenControl]),
                ...
                target: 'imageMap',
                view: new ol.View({
                    projection: proj, center: imgCenter, zoom: 3,
            ...
            this.resetBrightnessContrast = function() {
                brightness = ($stateParams.brightness)
                    ? ($stateParams.brightness)
                    : (0.0);
                contrast = ($stateParams.contrast)
                    ? ($stateParams.contrast)
                    : (1.0);
                source.refresh();
            };
            ...
            function addMeasureInteraction(measureType) {
                ...
                draw = new ol.interaction.Draw({
                    source: measureSource, type:/** @type {ol.geom.GeometryType} */
                    (type),
                    style: new ol.style.Style({
            ...
            var drawPqePoint;
            drawPqePoint = new ol.interaction.Draw({source: measureSource, type: 'Point'});
